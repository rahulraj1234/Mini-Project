          ...
          <div className='m-4 flex flex-col md:flex-row w-full p-4 rounded-lg border-3 shadow-lg bg-white lg:space-x-5 mb-8 h-full'>
            <div>
              ...
              <div className='flex gap-4 mt-8'>
                ...
                {!currentUser && <Link to='/sign-in'><FaHeart className="text-red-500 cursor-pointer group-hover:text-red-800" /></Link>}
                {currentUser && 
                  <div className="relative group">
                    <FaHeart onClick={handleWish} className="text-red-500 cursor-pointer group-hover:text-red-800" />
                    <span className="absolute top-full left-1/2 -translate-x-1/2 bg-gray-700 text-white text-sm px-2 py-1 rounded-md mt-2 opacity-0 transition-opacity duration-300 pointer-events-none group-hover:opacity-100">Wishlist</span>  
                  </div>}
                </div>
                {!currentUser && <p className="font-semibold text-green-600"><span className="hover: underline"><Link to='/sign-in'>Sign to book property</Link></span></p>}
              ...
              <div className="mt-8">
                ...
                {currentUser && <Comments listingId={listing._id} />}
                {currentUser && <div className="flex justify-center">
                  ...
                  {currentUser && <button onClick={handleSubmitRating} className="bg-red-900 text-white p-2 ml-4 rounded-md">Submit Rating</button>}
                </div>}
              </div>
            </div>
